...
from pyspark.mllib.linalg import Vectors, VectorUDT #spark<2.0
...
def multitransform(func):
    ...
    def multitransformer(dataframe, inputCols, outputCols, **kwargs):
        assert type(inputCols) == list and type(outputCols) == list
...
@multitransform
def binarizer(dataFrame, threshold=0.0, inputCol=None, outputCol=None):
...
@multitransform
def onehot(dataFrame, inputCol, outputCol, dropLast=False):
...
@multitransform
def standard_scaler(dataFrame, inputCol, outputCol, withStd=True, withMean=True):
...
@multitransform
def bucktizer(dataFrame, splits, inputCol, outputCol, handleInvalid='error'):
...
@multitransform
def discretizer(dataFrame, inputCol, outputCol, numBuckets=2, relativeError=0.001, handleInvalid='error'):
