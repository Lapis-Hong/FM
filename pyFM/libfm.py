...
class FM:
    ...
    def __init__(self,
        ...
        if sys.version < '2.7':
            c_githash = subprocess.Popen(['ls', '-l'], stdout=subprocess.PIPE).communicate()[0].strip()
        else:
    ...
    def run(self, train_set, test_set, validation_set=None, meta=None):
        ...
        self.save(preds, '%s/predictions' % self.__model_path)
        ...
        self.save(weights, '%s/latent vectors' % self.__model_path)  # call the statistic method
        self.save(pairwise_interactions, '%s/latent vectors' % self.__model_path)
